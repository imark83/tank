...
void loadData () {
	...
	GLfloat verticesTankTriangle[3*nVertices[tankTriangle]];

	generateVertices (verticesLines, verticesTankFace, verticesTankLines, verticesTankTriangle);
	...
	glBindBuffer (GL_ARRAY_BUFFER, buffer[tankTriangle]);
	glBufferData (GL_ARRAY_BUFFER, sizeof (verticesTankTriangle), verticesTankTriangle, GL_STATIC_DRAW);
	glVertexAttribPointer (0, 3, GL_FLOAT, GL_FALSE, 0, (const void*) 0);
	glEnableVertexAttribArray (0);
